...
void gd_usbd_reset_handler(void)
{
    /* open ep0 OUT and IN */
...
void gd_usbd_ep0_setup_handler(void)
{
    struct urequest _NULL;
    rt_usbd_ep0_setup_handler(&_gd_udc, (struct urequest *)&_NULL);
}

void gd_usbd_ep_in_handler(void)
{
    uint8_t epnum;
    ...
    {
        rt_usbd_ep_in_handler(&_gd_udc, 0x80 | epnum, hpcd->IN_ep[epnum].xfer_count);
    }
}

void gd_usbd_connect_handler(void)
{
    rt_usbd_connect_handler(&_gd_udc);
}

void gd_usbd_sof_handler(void)
{
    rt_usbd_sof_handler(&_gd_udc);
}

void gd_usbd_disconnect_handler(void)
{
    rt_usbd_disconnect_handler(&_gd_udc);
}

void gd_usbd_ep_out_handler(uint8_t epnum)
{
    if (epnum != 0)
    {
        rt_usbd_ep_out_handler(&_gd_udc, epnum, hpcd->OUT_ep[epnum].xfer_count);
    }
    else
    {
        rt_usbd_ep0_out_handler(&_gd_udc, hpcd->OUT_ep[0].xfer_count);
    }
}

void gd_usbd_set_connection_state(uint8_t state)
{
    if (state == 1)
    {
#if defined(SOC_SERIES_STM32F1)
