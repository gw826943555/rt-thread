...
static rt_err_t _ep_set_stall(rt_uint8_t address)
{
    ...
    {
        address &= 0x7F;
        USB_DIEPxCTL(address) |= DEPCTL_STALL;
    }
    else
    {
        USB_DOEPxCTL(address) |= DEPCTL_STALL;
    }
...
static rt_err_t _ep_clear_stall(rt_uint8_t address)
{
    ...
    if(address & 0x80)
    {
        address &= 0x7F;
        USB_DIEPxCTL(address) &= DEPCTL_STALL;
        USB_DIEPxCTL(address) |= DEPCTL_SD0PID;
    }
    else
    {
        USB_DOEPxCTL(address) &= DEPCTL_STALL;
        USB_DOEPxCTL(address) |= DEPCTL_SD0PID;
    }
...
static rt_err_t _ep_enable(uep_t ep_t)
{
    ...
    if (!(devepctl & DEPCTL_EPACT)) {
        ...
        devepctl |= (uint32_t)ep_t->ep_desc->type << 18U;
    ...
    ep->endp_mps = ep_t->ep_desc->wMaxPacketSize;
    ep->endp_type = ep_t->ep_desc->type;
...
{
    uint8_t addr = ep->ep_desc->bEndpointAddress;
    
    if(addr & 0x80)
    {
        if(USB_DIEPxCTL(addr & 0x7F) & DEPCTL_EPEN)
        {
            USB_DIEPxCTL(addr & 0x7F) |= DEPCTL_EPD;
        }
    }
    else
    {
        if(USB_DOEPxCTL(addr) & DEPCTL_EPEN)
        {
            USB_DOEPxCTL(addr) |= DEPCTL_EPD;
        }
    }
...
static rt_size_t _ep_write(rt_uint8_t address, void *buffer, rt_size_t size)
{
    ...
    else
    {
        ...
        else
        {
            ...
            USB_DIEPxLEN(ep_num) |= (1 << 19);
            USB_DIEPxLEN(ep_num) |= (DEPLEN_TLEN & ep->xfer_len);
        }
...
static void gd_usb_write_fifo(gd_usb_dev *dev, uint8_t ep_num)
{
    ...
    if (len > EP_MAX_PACKET_SIZE) {
        len = EP_MAX_PACKET_SIZE;
    }
...
static void gd_usb_poll(void)
{
    ...
    if(intsts & GINTF_OEPIF)
    {
        ...
        while(endp_intr)
        {
            if (endp_intr & 0x1U) 
            {
                ...
                {
                    USB_LOG("EP%d out:transfer complete\r\n", endp_num);
                ...
                if (out_endp_intr & DOEPINTF_STPF) 
                {
                    ...
                    USB_LOG("EP%d out:setup phase done\r\n", endp_num);
    ...
    if (intsts & GINTF_MFIF) 
    {
        ...
        USB_LOG("INT: mode mismatch.\r\n");
    }
    
    /* early suspend interrupt */
    if (intsts & GINTF_ESP) 
    {
        USB_LOG("INT: early suspend.\r\n");
...
//        USB_LOG("INT: start of frame.\r\n");
    ...
    {
        USB_LOG("INT: reveive fifo not empty.\r\n");
        usb_ep_struct *ep;
    ...
    if (intsts & GINTF_RST) 
    {
        ...
        _gd_usbd.in_ep[0].endp_mps = EP_MAX_PACKET_SIZE;
        _gd_usbd.in_ep[0].endp_type = USB_EP_ATTR_CONTROL;
        
        _gd_usbd.out_ep[0].endp_mps = EP_MAX_PACKET_SIZE;
        _gd_usbd.out_ep[0].endp_type = USB_EP_ATTR_CONTROL;
    ...
    {
        USB_LOG("INT: enumeration has been finished.\r\n");
    ...
    {
        USB_LOG("INT: incomplete synchronization in transfer.\r\n");
    ...
    {
        USB_LOG("INT: incomplete synchronization out transfer.\r\n");
        
        USB_GINTF |= GINTF_ISOONCIF;
    }
}

void gd_usb_thread(void* argu)
{
    rt_usb_device_init();
    while(1)
    {
